...
class SpaceRepository:
    ...
    def get_all(self):
        ...
        return [Space(row['id'], row['name'], row['description'], row['price_per_night'], row['owner_id']) for row in rows]
    
    def get_single_space(self, space_id):
        rows = self._connection.execute('SELECT * FROM spaces WHERE id = %s', [space_id])
        row = rows[0]
        return Space(row['id'], row['name'], row['description'], row['price_per_night'], row['owner_id'])
